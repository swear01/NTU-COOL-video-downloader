...
- 每支影片在 4 到 32 路之間自動調整分段下載數量，兩支合計最多 64 路。
- 完全在瀏覽器內合併 H.264 影像與 AAC 音訊。
- 完成後交給瀏覽器原本的下載管理器。
- 可貼上多個 NTU COOL 單支影片頁面連結，最多同時下載兩支。
- 提供英文與繁體中文介面。
...
Windows 請在 PowerShell 執行 `Get-FileHash .\NTU-COOL-video-downloader-1.2.7.zip -Algorithm SHA256`，並和 `SHA256SUMS` 比對。

使用 [GitHub CLI](https://cli.github.com/)驗證建置來源簽章：

```sh
gh attestation verify NTU-COOL-video-downloader-1.2.7.zip \
