...
小而美的 Chromium 擴充套件，用來把 NTU COOL 原生影片下載成 MP4。它直接使用瀏覽器中已開啟的登入狀態，不做登入自動化、不需要本機 helper，也不使用開發者代管伺服器。
...
其他下載類擴充功能（例如圖片或影片下載器）可以與本擴充功能並存。本擴充功能只在自己產生的 MP4 存檔期間註冊檔名監聽器，而且絕不會為自己未發起的下載建議檔名，因此不會覆蓋其他下載類擴充功能選擇的檔名。
...
批量下載時，點擊擴充套件圖示，在小視窗選擇「開啟 COOL 批量下載」；也可在圖示上按右鍵選擇同名入口。將單支影片頁面網址一行貼上一個，再按「開始」。「暫停」會暫停目前傳輸，「停止」會取消整個佇列。停止後仍保留成功與失敗紀錄。展開影片列可查看錯誤階段、HTTP 狀態、音／視訊軌、片段編號及嘗試次數；可「複製失敗網址」、「複製錯誤報告」或「匯出 JSON 報告」。自動複製失敗時會顯示可手動選取的文字框。「重試失敗影片」只為失敗影片取得全新授權，保留成功結果。批量模式會直接取得 COOL 授權及影片來源，不會逐支開啟影片分頁。最新報告會留在本機，重新開啟瀏覽器後仍可查看，但不會自動續傳。
...
| `https://*.dlc.ntu.edu.tw/*` | 存取臺大影片授權、影片資訊及媒體主機。 |
| 選用 `https://cool.ntu.edu.tw/*` | 只在使用者開始批量下載時請求，用既有登入狀態讀取貼上的頁面，取得全新的影片授權。 |

擴充套件不讀取或匯出 Cookie、密碼及一般瀏覽紀錄；瀏覽器會在授權的 COOL 請求中自動使用既有登入 Cookie。批量模式暫時解析 COOL 提供的 LTI 授權表單（可能包含姓名、電子郵件、帳號識別碼及授權簽章），只將表單送回官方影片服務，不持久保存表單或傳給開發者。簽章影片網址留在暫時的工作階段狀態，持久報告不包含這些網址。沒有分析、遙測、廣告或遠端程式碼。
...
Windows 請在 PowerShell 執行 `Get-FileHash .\NTU-COOL-video-downloader-1.2.4.zip -Algorithm SHA256`，並和 `SHA256SUMS` 比對。
